...
public class HdDetailAction extends BaseAction {
    ...
    public void showYunhaiDiskDetails() {
        ...
        //用diskId獲得制定id的硬盤
        DisksDetailReponse disksDetail = volumeClient.DescribeDisks(regionId.trim(), zoneId, "[" + diskId.trim() + "]",
        ...
        if (null == disksDetail.getMessage()) {
            ...
            switch (attachStatus) {
                case "attached":
                    DescribeInstancesResponse instanceResponse = instanceClient.DescribeInstances(regionId.trim(),zoneId ,
        ...
        if (null == diskResponse.getMessage()) {
            ...
            if (diskTypeItems.size() > 0) {
                for (InstanceTypeItem diskTypeItem : diskTypeItems) {
                    if (diskSize.equals(diskTypeItem.getHardDisk())) {
