...
(function(a) {
	a.fn.shCircleLoader = function(i, e) {
		...
		a.extend(o, i);
		var n = o.color, l = o.namespace, k = o.dots, f = o.externalCss, b = o.uaPrefixes, p = function(
				q) {
		...
		if (!f) {
			...
			if (!/(\;|\{)\s*visibility\s*\:/gi.test(g)) {
				g = /^(0+\%|from)\s*\{/i.test(g) ? g.replace(
						/^((0+\%|from)\s*\{)(.*)$/i, "$1visibility:visible;$3")
						: (/\s+(0+\%|from)\s*\{/i.test(g) ? g.replace(
								/(\s+(0+\%|from)\s*\{)/i,
								"$1visibility:visible;")
								: ("0%{visibility:visible}" + g))
			}
			a(a("head").get(0) ? "head" : "body").append(
					'<style id="'
							+ l
							+ c
							+ '" type="text/css">'
							+ j("@{prefix}keyframes " + l + c + "_bounce{" + g
									+ "}") + "</style>")
		}
		d
				.each(function() {
					var q, u, z, s, D, F, E, A, w, C, B = {}, t = a(this), v = t
							.find("." + m);
					if (v.get(0)) {
						v.shCircleLoader("destroy")
					}
					t.html('<div class="' + l + ((l != m) ? (" " + m) : "")
							+ '"></div>');
					if (f) {
						t = t.find("div")
					}
					F = t.innerWidth() - p(t.css("padding-left"))
							- p(t.css("padding-right"));
					E = t.innerHeight() - p(t.css("padding-top"))
							- p(t.css("padding-bottom"));
					q = (o.radius == "auto") ? ((F < E) ? (F / 2) : (E / 2))
							: o.radius;
					if (!f) {
						q--;
					...
					for (z = 0; z < k; z++) {
						...
						C = {
							left : Math.round(F + w) + "px",
							top : Math.round(E + w) + "px"
						};
						if (A) {
