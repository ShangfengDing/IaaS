...
public class SearchHdAction extends BaseAction {
	...
	private Logger logger = Logger.getLogger(HdListAction.class);//日志
	...
	public void counthd(){
		...
		volumes = volumeClient.searchByProperties(tenantId, uuid,
				userId, vmuuid, usageType, status, attachstatus, false,
				name, zid, null, null, null, null);

		for (int i = 0;i < volumes.size();i++){
			if(volumes.get(i).status.equalsIgnoreCase("available"))
			ava++;
		}
		try {
	...
	public String execute() {
		VmVolumeProxy vmVolumeProxy=ConnectionFactory.getVmVolumeProxy();

		Integer uid = null;
		logger.info("email:"+email);
		if(!email.equalsIgnoreCase("")){//根据email查找用户userid  equalsIgnoreCase忽略大小写
		...
		for (Volume volume : volumes) {
			try {
				VmVolume vmVolume=vmVolumeProxy.getByUUID(volume.id,false,false,false,false);
				userIdMap.put(volume.id,vmVolume.getUserId());
			}catch(Exception e){
				e.printStackTrace();
			}
			//查询硬盘所属的数据中心名称
			String zoneId = volume.availabilityZone;
			if (!zoneIdNameMap.containsKey(zoneId)) {//zoneIdNmaeMap里没有zoneId
				String zoneName = zoneClient.getZoneById(Integer.valueOf(zoneId)).name;
				zoneIdNameMap.put(zoneId, zoneName);   //添加
			}
