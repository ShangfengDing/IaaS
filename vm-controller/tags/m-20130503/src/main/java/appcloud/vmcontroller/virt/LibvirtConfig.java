    ...
    public void formatNetworkDevice() {
        for (int i = 0; i < networkConfigList.size(); i++) {
            ...
            Element netInterface = devices.addElement("interface")
                                          .addAttribute("type", networkConfigList.get(i).getNetworkType());
            // 采用桥接网络,如下配置:
            if (networkConfigList.get(i).getNetworkType().equals("bridge")) {
                ...
                netInterface.addElement("model").
                			 addAttribute("type", networkConfigList.get(i).getModelType());
                //设置防火墙，该版本注释掉防火墙设置部分
//            	netInterface.addElement("filterref")
//            				.addAttribute("filter", networkConfigList.get(i).getFilterName());
            
                //设置带宽限制
                if (networkConfigList.get(i).getOutbound() != null) {
                    ...
                    bandwidth.addElement("outbound")
                             ...
                             .addAttribute("burst", networkConfigList.get(i).getOutbound());
