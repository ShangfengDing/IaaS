...
public class EditImageAction extends BaseAction {
    ...
    public String publishImage(){
        ...
        appkey = appkeyManager.getAppkeyByUserIdAndAppName(userId, appname.trim());
        BaseResponse publishImgResponse = imageClient.authorizeImage(regionId,zoneId,imageId,appkey.getAppkeyId(),appkey.getAppkeySecret(),appkey.getUserEmail());
        return SUCCESS;
    }

    private void EditYunImg(Appkey appkey) {
        BaseResponse modifyImgResponse = imageClient.ModifyImageAttribute(regionId,zoneId, imageId, imageName, imageDes,imageSoftware, imageAccount, appkey.getAppkeyId(), appkey.getAppkeySecret(), appkey.getUserEmail());
