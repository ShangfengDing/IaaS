...
$(function(){
    ...
    $("#instancename").val(userEmail + time);
})
...
function sugstrByLen(str,leng){
    if(str.length>leng){
        var i=leng;
        while(str.lenght>leng){
            i--;
...
function acquireImageDetail(image,type,index) {
    var item = $(
        ...
        "<p>" + "操作系统:" + image.os + "</p>" +
        "<p>" + "默认密码:" + image.code + "</p>" +
        "<p>" + "镜像描述:" + sugstrByLen(image.description,144) + "</p>" +
...
function refreshProvider() {
    ...
    //    instance.provider = providerList[0];
    //}
    //$("#provider").html("");
    //for (var i = 0; i < providerList.length; i++) {
    //    var provider = providerList[i];
    //    var $item = $("<a class='btn btn-default front-no-box-shadow'>" + provider.providerName + "</a>")
    ...
    //}
    //
    //$("#instance-productor").html(instance.provider.providerName);
}
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result == "success") {
                    ...
                    $.fillTipBox({type:'warning', icon:'glyphicon-exclamation-sign', content:data.info})
                }
            },
            error:function(data){
...
function yunhai_term(){
    $('#termModal').modal('show');
    $('#termModal').css("zIndex", 1200);
}
