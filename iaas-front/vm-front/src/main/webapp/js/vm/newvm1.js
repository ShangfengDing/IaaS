...
function showvm(){
	...
	if(typeof(secId) == "undefined"){
		$("#secError").html("防火墙不得为空，请<a href='fw/fwlist'>点击这里</a>添加防火墙");
		$("#secError1").html("防火墙不得为空，请<a href='fw/fwlist'>点击这里</a>添加防火墙");
		return;
	} else{
		$("#secError1").html("");
	}
...
function newvm(){
	...
	if(name==""||des==""||zoneId==""||secId==""||
		...
		type==""||price==""||paymentType==""||count==""||agreement==""){
		fillTipBox("error","页面上有参数错误，请重新检查");
		alert("error");
		return;
	}
	showLoading();
	$.post("vm/newvm",{name:name, displayDescription:des, clusterId:clusterId, zoneId:zoneId, imgUuid:imgUuid, 
