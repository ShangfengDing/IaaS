...
public class SpecificationUtil<T> {
     ...
     * @return
     */
    public Specification<T> where(final Map<String, QueryObject> searchMap) {
            ...
            public Predicate toPredicate(Root root, CriteriaQuery criteriaQuery, CriteriaBuilder criteriaBuilder) {
                ...
                for (Map.Entry searchObjct: searchMap.entrySet()) {
                    try {
                        ...
                        switch (type) {
                            case EQUAL:
                                list.add(criteriaBuilder.equal(root.get(key).as(clazzs[0]), value.getValue()[0]));
                                break;
                            case NOT_EQUAL:
                                list.add(criteriaBuilder.notEqual(root.get(key).as(clazzs[0]), value.getValue()[0]));
                                break;
                            case BOTH_LIKE:
