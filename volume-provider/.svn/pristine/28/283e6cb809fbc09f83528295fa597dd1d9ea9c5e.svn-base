...
public class Conf {
	...
	public static final String IMG_SERVER_PATH = "/nfs/192.168.1.57/";
...
//			VOLUME_PROVIDER_HOST = IpUtil.getIpv4Address();			
	...
	public static void initMountFs(String nfsHost) throws IOException {
		if(nfsHost != null && !"".equals(nfsHost)) {
			if (!NFSUtil.isMounted(nfsHost)) {
				...
				if (result) {
					...
					throw new IOException("mount " + nfsHost + ":"
							+ NFSUtil.DEFAULT_NFS_SRV_PATH + " failed!");
				}
			} else {
				logger.info("mount " + nfsHost + " already be mounted!");
			}
		}
	}
}
