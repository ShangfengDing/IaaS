...
public abstract class BaseFilter {
	public List<Host> filterAll(List<Host> hostList, VmInstance instance, ServiceTypeEnum serviceTypeEnum){
		List<Host> resultHostList = new ArrayList<Host>();
		if(hostList == null){
			hostList = getLivingHost(instance, serviceTypeEnum);
		}
		for(Host host : hostList){
			if(filterOne(host, instance)){
				resultHostList.add(host);
			}
		}
		return resultHostList;
	}
	protected abstract boolean filterOne(Host host, VmInstance instance);
