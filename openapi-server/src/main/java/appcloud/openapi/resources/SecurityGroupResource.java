...
public class SecurityGroupResource extends BaseController{
	...
	public CreateSecurityGroupResponse create(@RequestParam(value = Constants.ACTION) String action, 
			...
			try {
				...
				SecurityGroup securityGroup = securityGroupOperate.create(appKeyId, regionId, paramsMap.get(Constants.SECURITY_GROUP_NAME), paramsMap.get(Constants.DESCRIPTION), requestId);
				
				return new CreateSecurityGroupResponse(requestId, String.valueOf(securityGroup.id));
			} catch (Exception e) {
