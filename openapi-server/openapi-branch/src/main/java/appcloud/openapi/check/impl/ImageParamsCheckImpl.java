...
public class ImageParamsCheckImpl implements ImageParamsCheck{
    ...
    public Map<String, String> checkCreateParams(Map<String, String> paramsMap) throws Exception {
        logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check create image params, RegionId=" + paramsMap.get(Constants.REGION_ID));
        ...
        if(paramsMap.containsKey(Constants.DISPLAY_NAME)) {
        	if(!StringUtil.isValidInSizeAndChinese(paramsMap.get(Constants.DISPLAY_NAME), 
    ...
    @Override
    public Map<String, String> checkAuthorizeParams(Map<String, String> paramsMap) throws JSONException {
    ...
    public Map<String, String> checkUpdateParams(Map<String, String> paramsMap) throws Exception {
        ...
        if(!paramsMap.containsKey(Constants.DISPLAY_NAME) || null==paramsMap.get(Constants.DISPLAY_NAME)) {
        	...
        	if(!StringUtil.isValidInSizeAndChinese(paramsMap.get(Constants.DISPLAY_NAME), Constants.MIN_NAME, Constants.MAX_NAME)) {
                return commonGenerator.inValid(null, Constants.DISPLAY_NAME);
            }
            /*if(vmImageProxy.getByUuid(paramsMap.get(Constants.IMAGE_ID)).getOsType().toString().equalsIgnoreCase(Constants.OS_WINDOWS)) {
            ...
            }else if(vmImageProxy.getByUuid(paramsMap.get(Constants.IMAGE_ID)).getOsType().toString().equalsIgnoreCase(Constants.OS_LINUX)){
                if(paramsMap.get(Constants.DISPLAY_NAME).length()<Constants.MIN_HOSTNAME || 
        ...
        if(!paramsMap.containsKey(Constants.DISPLAY_DESCRIPTION) || null==paramsMap.get(Constants.DISPLAY_DESCRIPTION)) {
            ...
            if(!StringUtil.isValidInSizeAndChinese(paramsMap.get(Constants.DISPLAY_DESCRIPTION), Constants.MIN_DESCRIPTION, Constants.MAX_DESCRIPTION)) {
      ...
      //判断非必须参数
        ...
        switch (type) {
            ...
            case Constants.IMAGE_PRIVATE:
                break;
            default:
                return commonGenerator.inValid("the type is invalid", Constants.TYPE);
        }
        Map<String, String> resultMap = new HashMap<String, String>();
        resultMap.put(Constants.HTTP_CODE, HttpConstants.STATUS_OK);
        return resultMap;
