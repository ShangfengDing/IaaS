...
//		else
	...
	public VmUserItem VmUserToVmUserItem(VmUser vmUser) {
		if (vmUser == null)  return null;
		return new VmUserItem(vmUser.getPreId(), vmUser.getUserId(), vmUser.getUserEmail(), vmUser.getActive(), vmUser.getGroupId(), vmUser.getAppKeyId(), vmUser.getAppKeySecret(), vmUser.getEnterpriseId());
	}

	public AppkeyItem AppkeyToAppkeyItem(Appkey appkey) {
		if (appkey == null)  return null;
		return new AppkeyItem(appkey.getId(), appkey.getUserId(), appkey.getUserEmail(), appkey.getAppkeyId(), appkey.getAppkeySecret(),
				appkey.getProvider(), appkey.getAppname(), appkey.getRegion(), appkey.getZone(), appkey.getAppcloud_userEmail(), appkey.getState());
	}
	...
	public ImageBackItem VmImageBackToImageBackItem(VmImageBack imageBack) {
		String imageStatus = imageBack.getImageStatus()==null?"avaliable":imageBack.getImageStatus().toString();
		String volumeType = imageBack.getVolumeType() ==null? "qcow2":imageBack.getVolumeType().toString();
		String usageType = imageBack.getUsageType() == null? "system":imageBack.getUsageType().toString();
		return new ImageBackItem(imageBack.getId(), imageBack.getVolumeUuid(), imageBack.getDisplayName(), imageBack.getUserId(), imageBack.getAvailabilityZoneId(),
				imageBack.getAvailabilityClusterId(), imageBack.getParentVolumeUuid(), imageBack.getSonVolumeUuid(), imageBack.getActiveVolumeUuid(), imageBack.getInstanceUuid(),
				imageBack.getProviderLocation(), imageBack.getHostUuid(), imageBack.getCreatedTime(), imageBack.getUpdatedTime(), imageBack.getDeletedTime(), imageBack.getVolumeSize(),
				imageBack.isBackUp(), imageBack.isTop(), imageStatus, volumeType, usageType);
	}
