...
public class CommonResource extends BaseController{
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(method = RequestMethod.GET,  params="Action="+ActionConstants.GAIN_APPKEY_PAIR, produces = { "application/json","application/xml"})
	@ResponseBody
	public GainAppkeyPairResponse GainAppkeyPair(@RequestParam(Constants.ACTION) String action, 
		...
		try {
			...
			String userIdTemp = resultMap.get(Constants.USER_ID);
			Integer userId = userIdTemp==null?null:Integer.valueOf(userIdTemp);
			return new GainAppkeyPairResponse(requestId, resultMap.get(Constants.APPKEY_ID),
					resultMap.get(Constants.APPKEY_SECRET), userId, null, null);
		} catch (Exception e) {
