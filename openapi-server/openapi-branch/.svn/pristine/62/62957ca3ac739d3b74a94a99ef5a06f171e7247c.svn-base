...
//		String regionId = ( vmZoneProxy.getByRegionId(paramsMap.get(Constants.REGION_ID)) ) .getId() + "";
		String availableZoneId = String.valueOf(vmZoneProxy.getByZoneId(paramsMap.get(Constants.ZONE_ID)).getId());
		...
		ServerCreateReq serverCreateReq = new ServerCreateReq(
				paramsMap.get(Constants.IMAGE_ID), flavorRef, name, availableZoneId, userGroup.getAvailableClusters(), metadata, securityGroup);
		return serverCreateReq;
	}

	public ServerCreateReq gainServerRecoveryReq(String userId, Map<String, String> paramsMap) throws Exception {
		logger.info("gain create instance request, userId=" + userId);
		VmUser user = vmUserProxy.getByUserId(Integer.valueOf(userId));
		if(null==user) {
			return null;
		}
		VmGroup userGroup = vmGroupProxy.getById(user.getGroupId());
...
//		}
		 ...
		 * 由于云主机名称和描述不是接口必填字段，故首先将云主机的名称和描述暂时置为用户userId，等云主机创建成功后，再将其修改过来
		 */
		Flavor newFlavor = instanceOperate.createRecoveryFlavor(user.getUserId(), paramsMap.get(Constants.INSTANCE_TYPE),
				Integer.parseInt(paramsMap.get(Constants.DATA_DISK1_SIZE)), paramsMap.get(Constants.IMAGEBACK_ID) );
		logger.info("用户  " + user.getUserId() + " 第三方创建flavor成功");
		String flavorRef = newFlavor.id.toString();	//flavorRef传的是flavor的id
		String regionId = ( vmZoneProxy.getByRegionId(paramsMap.get(Constants.REGION_ID)) ) .getId() + "";
		String name = paramsMap.get(Constants.INSTANCE_NAME) == null ? user.getUserId()+"" : paramsMap.get(Constants.INSTANCE_NAME);
		...
		metadata.put(ServerMetadata.PRI_BANDWIDTH, ConnectionConfigs.PRIVATE_MAX_BANDWIDTH.toString());
		metadata.put(ServerMetadata.DISPLAY_DESCRIPTION, description);
		metadata.put(ServerMetadata.VM_MAC_PRI, paramsMap.get(Constants.VM_MAC_PRI));
		metadata.put(ServerMetadata.VM_MAC_PUB, paramsMap.get(Constants.VM_MAC_PUB));
		metadata.put(ServerMetadata.IMAGE_ID, paramsMap.get(Constants.IMAGE_ID));
		//选择防火墙
		SecurityGroup securityGroup = new SecurityGroup();
		securityGroup.id = vmSecurityGroupProxy.getByUuid(paramsMap.get(Constants.SECURITY_GROUP_ID), false).getId();
		ServerCreateReq serverCreateReq = new ServerCreateReq(
				paramsMap.get(Constants.IMAGEBACK_ID), flavorRef, name, regionId, userGroup.getAvailableClusters(), metadata, securityGroup);
		return serverCreateReq;
	}
