...
public class InstanceParamsCheckImpl implements InstanceParamsCheck{
	...
	private static VmImageBackProxy vmImageBackProxy;
	...
	public Map<String, String> checkCreateParams(Map<String, String> paramsMap) throws Exception {
		...
		if(null!=paramsMap.get(Constants.HOST_NAME) ) {
			if(Constants.OS_WINDOWS.equalsIgnoreCase(vmImageProxy.getByUuid(paramsMap.get(Constants.IMAGE_ID)).getOsType().toString())) {
			...
			}else if(Constants.OS_LINUX.equalsIgnoreCase(vmImageProxy.getByUuid(paramsMap.get(Constants.IMAGE_ID)).getOsType().toString())){
				if(paramsMap.get(Constants.HOST_NAME).length()<Constants.MIN_HOSTNAME || 
		...
		if(null!=paramsMap.get(Constants.PASSWORD) &&
			...
			return commonGenerator.inValid("password should be character with legnth between "+Constants.MIN_PASSWORD+" to "+ Constants.MAX_PASSWORD, Constants.PASSWORD);
		}
		...
		//无需检查，系统盘采用镜像的大小创建，以后系统盘扣费放在镜像扣费处，Billingrate的harddisk不再使用
	...
	public Map<String, String> checkRecoveryParams(Map<String, String> paramsMap) throws Exception {
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check create instance params, RegionId=" + paramsMap.get(Constants.REGION_ID) +
				"; ImageBackId=" + paramsMap.get(Constants.IMAGEBACK_ID) + "; InstanceType=" + paramsMap.get(Constants.INSTANCE_TYPE) +
				"; SecurityGroupId=" + paramsMap.get(Constants.SECURITY_GROUP_ID));
		if(null==paramsMap.get(Constants.REGION_ID)) {
			return commonGenerator.missing(null, Constants.REGION_ID);
		}else {
		...
		if(null==paramsMap.get(Constants.IMAGEBACK_ID)) {
			return commonGenerator.missing(null, Constants.IMAGEBACK_ID);
		}else {
			if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.IMAGEBACK_ID))) {
				return commonGenerator.inValid(null, Constants.IMAGEBACK_ID);
			}else if( null==vmImageBackProxy.getByUuid(paramsMap.get(Constants.IMAGEBACK_ID))) {
				return commonGenerator.operationDeny(null, Constants.IMAGEBACK_ID);
			}
		}
		if (StringUtil.isEmpty(paramsMap.get(Constants.VM_MAC_PRI))) {
			return commonGenerator.missing(null, Constants.VM_MAC_PRI);
		}

		if (StringUtil.isEmpty(paramsMap.get(Constants.VM_MAC_PUB))) {
			return commonGenerator.missing(null, Constants.VM_MAC_PUB);
		}
...
//		}
	...
	@Override
	public Map<String, String> checkMonitorParams(Map<String, String> paramsMap)
			throws Exception {
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check modify instance charge type params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
		...
		if(null!=paramsMap.get(Constants.PERIOD)) {
			if(!"day".equalsIgnoreCase(paramsMap.get(Constants.PERIOD)) && !"month".equalsIgnoreCase(paramsMap.get(Constants.PERIOD))
				&&	!"minute".equalsIgnoreCase(paramsMap.get(Constants.PERIOD))) {
				return commonGenerator.inValid(null, Constants.PERIOD);
			}
		} 
		Map<String, String> resultMap = new HashMap<String, String>();
		resultMap.put(Constants.HTTP_CODE, HttpConstants.STATUS_OK);
		logger.info("Check get instance monitor infos params successfully");
		return resultMap;
	}
	
	@Override
	public Map<String, String> checkMigrateParams(Map<String, String> paramsMap)
			throws Exception {
		// TODO Auto-generated method stub
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check migrate instance params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
			return commonGenerator.missing(null, Constants.INSTANCE_ID);
		}else if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.INSTANCE_ID)) ){
		...
		}else if(!Pattern.matches("\\w{12}", paramsMap.get(Constants.HOST_ID)) ){
				return commonGenerator.inValid(null, Constants.HOST_ID);
		}else if(null==hostProxy.getByUuid(paramsMap.get(Constants.HOST_ID), 
				false, false, false)){
				return commonGenerator.notFound(null, Constants.HOST_ID);
		}
		Map<String, String> resultMap = new HashMap<String, String>();
		resultMap.put(Constants.HTTP_CODE, HttpConstants.STATUS_OK);
		return resultMap;
	...
	@Override
	public Map<String, String> checkOnlineMigrateParams(
			Map<String, String> paramsMap) throws Exception {
		// TODO Auto-generated method stub
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check online migrate instance params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
			return commonGenerator.missing(null, Constants.INSTANCE_ID);
		}else if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.INSTANCE_ID)) ){
	...
	@Override
	public Map<String, String> checkSuspendParams(Map<String, String> paramsMap)
			throws Exception {
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check suspend instance params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
			return commonGenerator.missing(null, Constants.INSTANCE_ID);
		}else if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.INSTANCE_ID)) ){
	...
	@Override
	public Map<String, String> checkResumeParams(Map<String, String> paramsMap)
			throws Exception {
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check resume instance params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
			return commonGenerator.missing(null, Constants.INSTANCE_ID);
		}else if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.INSTANCE_ID)) ){
	...
	public static void main(String[] args) throws ParseException {
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
		d = dateFormat.parse("2016-04-20T14:10:00Z");
	...
	public Map<String, String> checkDescInstancesParams(Map<String, String> paramsMap) throws Exception {
		...
		if(null!=paramsMap.get(Constants.INSTANCE_STATUS)) {
			try {
				VmStatusEnum.valueOf(paramsMap.get(Constants.INSTANCE_STATUS));
			} catch (Exception e) {
	...
	public Map<String, String> checkRenewInstanceParams(
			Map<String, String> paramsMap) throws Exception {
		logger.info("Action: "+paramsMap.get(Constants.ACTION)+" check Renew instance params, InstanceId=" + 
				paramsMap.get(Constants.INSTANCE_ID) );
		if(null==paramsMap.get(Constants.INSTANCE_ID)) {
			return commonGenerator.missing(null, Constants.INSTANCE_ID);
		}else if(!Pattern.matches("\\w{32}", paramsMap.get(Constants.INSTANCE_ID)) ){
		...
		Map<String, String> resultMap = new HashMap<String, String>();
		resultMap.put(Constants.HTTP_CODE, HttpConstants.STATUS_OK);
		logger.info("Check Renew instance params successfully!");
		return resultMap;
	}
