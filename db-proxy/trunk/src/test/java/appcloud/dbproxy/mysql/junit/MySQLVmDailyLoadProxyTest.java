...
import appcloud.dbproxy.mysql.MySQLDailyLoadProxy;
...
public class MySQLVmDailyLoadProxyTest {

	private static final String HOST_UUID = "host-load";
	private static MySQLDailyLoadProxy proxy;
	
	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		proxy = new MySQLDailyLoadProxy();
	...
	public void testGetOneDay() {
		...
		HostLoad load = proxy.getOneDayLoad(HOST_UUID, new Timestamp(date.getTime() - 80*ONE_DAY_MINISECONDS));
		assertNull(load);
		
		load = proxy.getOneDayLoad(HOST_UUID, new Timestamp(date.getTime() - 5*ONE_DAY_MINISECONDS));
		assertEquals(0.1, load.getAvgLoad(), 0.001);
	...
	public void testGetAvgLoad() {
		...
		assertEquals(startTime, list.get(0).getTime());
	}
