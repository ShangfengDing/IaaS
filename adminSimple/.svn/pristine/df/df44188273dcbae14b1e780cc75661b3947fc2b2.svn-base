$(document).ready(function(){
    ...
    /*$("#admin_button").find("label").on('click',function(){
...
function deleteAdminRole(id){
    $.tipModal('confirm', 'info', '确认删除该管理组吗？', function (result) {
        if (result == true) {
            $.ajax({
                type: "post",
                url: "runtime/deleteAdminRole",
                ...
                success: function (data) {
                    if (data.result == "success") {
                        ...
                        facebox_close();
                        hideLoading();
                        $.fillTipBox({type:'warning', icon:'glyphicon-exclamation-sign', content:'正在使用中，无法删除'});
                    } else {
                        facebox_close();
                        hideLoading();
